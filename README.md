...
Терминальный монитор системы в реальном времени (CPU/GPU/RAM) с цветовой индикацией и графиками.

## Возможности

- Обновление метрик раз в секунду.
...
- Любые другие строки игнорируются.
...
- копирует `hwpulse.py` в `/opt/hwpulse/hwpulse.py`;
- создаёт launcher `/usr/local/bin/hwpulse`.
